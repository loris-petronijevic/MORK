...
fn main() {
    ...
    let solution = |t: f64| {
        vec![
            vec![-0.5 * ((-t).exp() + t.cos()), 0.5 * ((-t).exp() - t.sin())],
            vec![t.cos()],
        ]
    };
    ...
    let error = vec![
        vec![(exact[0][0] - y[0][0]).abs(), (exact[0][1] - y[0][1]).abs()],
        vec![(exact[1][0] - y[1][0]).abs()],
    ];

    println!(
        "Final approximations at {} are : {:?}",
        h * iterations as f64,
        y
    );
